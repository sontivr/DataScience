MakeWordLists <- function(wlists = c("gi","afinn","liu")){
  ...
  if("liu" %in% wlists){
    ...
    liu.pos <- read.delim("https://www.github.com/schaunwheeler/tmt/master/liu_positive_words.txt", 
      header=F, as.is=T, skip=35)$V1
    liu.neg <- read.delim("https://www.github.com/schaunwheeler/tmt/master/liu_negative_words.txt", 
      header=F, as.is=T, skip=35)$V1
  }
